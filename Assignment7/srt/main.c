...
/*Define the global for scene and nthreads*/
scene_t scene;
int nthreads;

/*Number of colors to be the depth of our 3D array*/
#define NUM_COLORS 3
...
main( int argc, char **argv )
{
  ...
  /*Create thread ID array and processes image*/
  ...
  for(int t = 0; t < nthreads; t++)
    {
      ...
      if(ret)
	{
	  fprintf(stderr, "Error freeing threads.");
	  exit(-1);
	}
    }
